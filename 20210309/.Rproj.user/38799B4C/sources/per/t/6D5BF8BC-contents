...
final_training <- bake(beer_recipe, NULL) %>%
  janitor::clean_names() %>%
  mutate(review_overall = as.factor(review_overall))
...
set.seed(69420)
beer_resamples <- vfold_cv(final_training, v = 6)
...
# doPar seems to be determined by the number of folds in the
#         cross validation

# grid 10, vfold = 10, par 6 works
# grid 10, vfold = 4, par 6 doesn't work
# grid 10, vfold = 6, par 6 does work
...
#          review_overall = as.numeric(as.character(review_overall))) %>%
#   group_by(id) %>%
#   summarize(rmse = mean((review_overall - .pred_class)^2)^(1/2)) %>%
#   ungroup() %>%
#   arrange(rmse)
#
# gbm_grid %>%
#   collect_predictions() %>%
#   ggplot(aes(x = .pred_class, y = review_overall, alpha = 0.1)) +
#     geom_point() +
#     geom_smooth() +
#     facet_wrap(~id) +
